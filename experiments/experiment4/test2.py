...
def extremes(out, plain):
    ...
    The first return value is the reading furthest from 0 among the rows where
    the predicate is false, and the second and third are the readings furthest
    from and closest to 1 among the rows where it is true. Every other row is
...
def report(name, out, plain):
    """Print the worst decrypted value on each side of the decision."""
...
slot_count = 32768
row_count = len(car_data)          # 16 recorded rows; the rest of the slots are padding
...
# Speed is recorded for the 11 rows before impact and marked -1 afterwards.
# The speed predicates are evaluated on the rows that carry a speed, since
# asking whether an unrecorded speed rose is not a question the log can answer.
...
MAX_SPEED = 200.0                  # normalization constant used by the speed predicates
THRESHOLD_SPEED = 60.0

# Query window, in the log's own unit (ms before impact). The bounds are chosen
# so that neither of them coincides with a recorded timestamp; a bound sitting
# exactly on a sample asks whether a value is strictly less than itself, which is
# not a well-posed query. This window also places the padding value -1 outside
# the window, so the padded slots answer 0 and every one of the 32,768 slots can
# be checked against the plaintext answer.
START = -3000.0
END = -1000.0
RANGE = 5000.0
...
# Plaintext answer. np.roll(speed, 1)[i] is speed[i-1], which is what the
# one-slot rotation inside detect_speed_increase computes.
START_TIME = time.time()
...
#===========================================#
START_TIME = time.time()
result2 = hft.detect_overspeed(speed_ctxt, THRESHOLD_SPEED)
...
#=====================================#
START_TIME = time.time()
result3 = hft.time_range(t_real_ctxt, START, END, RANGE)
...
##  8. Per-row answers         ##
#===============================#
# The log is short enough to print in full. A dot marks a row with no recorded
# speed, which the speed predicates do not cover.
...
).to_csv('results/exp4_avante_summary.csv', index=False)
...
}).to_csv('results/exp4_avante_rows.csv', index=False)

bad = (agree1 != speed_rows) + (agree2 != speed_rows) + (agree3 != slot_count)
print("\n 3 predicates in total,", bad, "mismatched")
print("Saved: results/result2.txt, results/exp4_avante_summary.csv, "
      "results/exp4_avante_rows.csv")
